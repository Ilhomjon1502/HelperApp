...
//                sendSms()
                try {
                    ...
                    binding.tvInfo.text = "Iltimos avval GPS va internetni yoqib oling, aks holda dastur to'liq ishlamaydi"
                    val phoneNumber = MySharedPreference.numberHelp // Telefon numarası burada belirtilmelidir
                    val message =
                        "${MySharedPreference.name} ga yordam kerak, ${MySharedPreference.number}." // Gönderilecek mesaj burada belirtilmelidir

                    val smsManager = SmsManager.getDefault()
                    smsManager.sendTextMessage(phoneNumber, null, message, null, null)
                    Toast.makeText(context, "Faqat SMS xabar yuborildi", Toast.LENGTH_SHORT).show()
                }
                true
            }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.menu_info -> {
                ...
                dialog.setMessage("Bu dastur MVP bo'lib SAYDAKRAM va BURHONIDDIN ning g'oyasi bo'yicha ishlab chiqilgan.")
                dialog.show()
            }
            R.id.menu_edit -> {
    ...
    fun myLocation() {
        ...
        if (latitude != null && longitude!= null){
            ...
            requestRetrofit(MyHelp(1, MySharedPreference.name, latitude.toString(), longitude.toString(), MySharedPreference.number, MySharedPreference.sana.toInt()))
        }else {
            ...
            locationTask.addOnSuccessListener { it: Location? ->
                if (it != null) {
                    ...
                    requestRetrofit(
                        MyHelp(
                            1,
                            MySharedPreference.name,
                            it.latitude.toString(),
                            it.longitude.toString(),
                            MySharedPreference.number,
                            MySharedPreference.sana.toInt()
                        )
                    )
